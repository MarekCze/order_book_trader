...
Volatility filter (global filter 3) — redesigned in tuning iteration 1: the gate uses a 30-minute "regime" ATR (a separate series from F34's 5-min ATR) sampled at CONTEXT formation, not at the signal trigger (`Risk:AtrSampleAtContext`), ranked against the trailing 20-day distribution. It is disabled (pass-through, logged via the CLI) until the trailing baseline holds `Risk:MinBaselineSessions` (default 10) sessions — a percentile over fewer sessions is meaningless, and the original trigger-time 5-min gate blocked ~100% of candidates because it sampled the very burst the setups react to. The [20th,95th] band keys are unchanged. See the rulebook annotation on filter 3.
